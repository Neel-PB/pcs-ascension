...
const columns: ColumnDef<any>[] = [
  ...
  { id: "SkillMix", label: "Skill Mix", type: "custom", width: 180, minWidth: 160, sortable: true, resizable: false, draggable: true, renderCell: (row) => <TruncatedTextCell value={row.skillMix} maxLength={30} /> },
  { id: "vacancyAge", label: "Vacancy Age", type: "custom", width: 180, minWidth: 170, sortable: true, resizable: false, draggable: true, getValue: (row) => getVacancyAge(row.positionStatusDate), renderCell: (row) => { const days = getVacancyAge(row.positionStatusDate); const badge = getVacancyBadge(days); return <BadgeCell value={badge.label} variant={badge.variant} className={badge.className} maxLength={30} />; }, sortFn: (a, b) => { const aDays = getVacancyAge(a.positionStatusDate) ?? 0; const bDays = getVacancyAge(b.positionStatusDate) ?? 0; return aDays - bDays; } },
...
export function ContractorRequisitionTab({
  selectedRegion, selectedMarket, selectedFacility, selectedDepartment,
}: ContractorRequisitionTabProps) {
    ...
    <div className="flex flex-col gap-4 min-h-0 max-h-full overflow-hidden">
      ...
      {isFetching ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center min-h-[300px] bg-muted/20 rounded-xl border border-border/50">
          <p className="text-muted-foreground">No contractor requisitions found.</p>
        </div>
      ) : (
      ...
      <PositionsFilterSheet open={filterOpen} onOpenChange={setFilterOpen} filters={filters} onFiltersChange={setFilters} onClearFilters={clearFilters} activeFilterCount={activeFilterCount} skillMixOptions={skillMixOptions} employeeTypeOptions={employeeTypeOptions} title="Filter Contractor Requisitions" />
    </div>
  );
}
