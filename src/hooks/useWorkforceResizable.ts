...
interface UseWorkforceResizableOptions {
  minWidth: number;
  maxWidthVw: number;
  snapPoints?: number[];
}

export const useWorkforceResizable = ({ minWidth, maxWidthVw, snapPoints = [] }: UseWorkforceResizableOptions) => {
  const { width, setWidth } = useWorkforceDrawerStore();
